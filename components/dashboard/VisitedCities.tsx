...
export default function VisitedCities({ cities }: { cities: City[] }) {
  if (cities.length === 0) {
    return (
      <div className="text-center py-16">
        <p className="text-gray-400">No visited cities yet.</p>
        <p className="text-xs text-gray-300 mt-1">Mark cities as visited from the explore page or any city page.</p>
        <Link href="/" className="mt-3 inline-block text-green-500 text-sm font-medium hover:underline">
          Browse cities
        </Link>
      </div>
    );
      ...
      {cities.map((city) => (
        ...
        >
          ...
          <div className="p-3">
            ...
            <p className="text-xs text-gray-400 mt-2">{city.bestSeason}</p>
          </div>
        </Link>
      ))}
    </div>
